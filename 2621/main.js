...
async function sleep(millis) {
  // Define a function that will be used as the executor for the Promise.
  // It takes `resolve` and `reject` callbacks as arguments.
  function callback(resolve, reject) {
    // Use setTimeout to wait `millis` milliseconds before calling resolve()
    setTimeout(() => resolve(), millis);
  }

  // Return a new Promise that resolves after `millis` milliseconds
